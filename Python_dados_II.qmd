...
```{mermaid}
%%| label: fig-tidy
%%| fig-cap: "Transformando wide (largo) em long (longo) data, e vice-versa."

graph TD
    A[Wide Data] --melt--> B[Long Data]
    B --pivot_table--> A
```
...
Nesta subseção, exploraremos novamente o fato de que o [vetor devolvido está na mesma ordem que nossos dados]{.underline}. [Podemos concatenar o novo vetor aos nossos dados originais]{.underline}.

É possível obter o mesmo resultado com um único passo tirando proveito do fato de que o resultado da separação devolve uma lista com dois elementos, em que cada elemento e uma nova coluna.

Podemos combinar a lista de itens separados com a função embutida `zip`, `zip` aceita um conjunto de iteradores (por exemplo, listas, tuplas) e cria um contêiner composto dos iteradores de entrada, mas cada novo contêiner criado tem o mesmo índice que os contêineres de entrada.

Por exemplo, se tivermos duas listas de valores,
...
podemos executar `zip` nas duas:

```{python}
# Temos que chamar list na função zip
# para exibir o conteúdo do objeto zip;
# Em Python 3, zip devolve um iterador
print(list(zip(constants,values)))
```

[Cada elemento agora tem a constante correspondente ao seu valor]{.underline}.

Conceitualmente, cada contêiner é como um lado de um ziper. Quando executamos `zip` nos contêineres, há uma correspondência entre os índices, e esses dados são devolvidos.

Outra forma de visualizar o que o `zip` faz é tomar cada contêiner passado para ele e empilhá-los uns sobre os outros (pense na concatenação por linhas), [criando, assim, uma espécie de dataframe]{.underline}. `zip` então devolve os valores coluna a coluna em uma tupla.

\newpage

Podemos usar o mesmo `ebola_long.variable.str.split('_')` para separar os valores da coluna. No entanto, como o resultado já é um contêiner (um objeto `Series`), temos que desempacotá-lo para obter o seu conteúdo (cada lista status-país), em vez de termos o próprio contêiner (a série).

Em Python, o operador asterisco, *, é usado para desempacotar contêineres. Quando executamos `zip` nos contêineres desempacotados, o efeito será o mesmo obtido quando criamos os valores de status e de país anteriormente. Podemos, então, atribuir os vetores às colunas simultaneamente usando a atribuição múltipla.
...
A lógica funciona assim:
   ...
   * `split('_')`: Quebra a string (ex: "Cases_Guinea") em uma lista: `['Cases', 'Guinea']`.

   * O Asterisco (`*`): Ele "abre a caixa". Em vez de passar a coluna inteira para o `zip`, ele passa cada lista individualmente.

   * O `zip`: Ele funciona como um zíper de jaqueta. Ele pega o primeiro item de cada lista e agrupa (todos os "Status"), depois pega o segundo item de cada lista e agrupa (todos os "Países").

\newpage

Aprofundando a função `zip`:

Imagine que o `zip` é um grampeador de listas. Ele pega o primeiro item de cada lista e grampeia juntos, depois o segundo, e assim por diante.
...
# O zip vai unir o 1º com o 1º, e o 2º com o 2º
resultado = list(zip(nomes, idades))

print(resultado)
```

Por que o texto usa o asterisco `zip(*...)`?

   * No seu caso do Pandas, você não tem duas listas separadas no início. Você tem uma única coluna onde cada linha é uma lista, tipo assim:

  ```[['Cases', 'Guinea'], ['Deaths', 'Guinea']].```

   * Se você der o `zip` direto nessa coluna, ele não sabe o que fazer. O asterisco (*) "derrama" o conteúdo dessa coluna.

   * Imagine que o asterisco faz isso:
   
   ```zip(['Cases', 'Guinea'], ['Deaths', 'Guinea'])```

   * O zip então "grampeia":

     * Pega todos os primeiros itens (índice 0): ('Cases', 'Deaths') -> Vai para a coluna Status.

     * Pega todos os segundos itens (índice 1): ('Guinea', 'Guinea') -> Vai para a coluna Country.

\newpage

## Variáveis tanto em linhas quanto em colunas

As vezes, os dados estarão formatados de modo que as variáveis se encontrarão tanto nas linhas quanto nas colunas - isto é, em alguma combinação de formatos descritos nas seções anteriores deste capítulo. A maioria dos métodos necessários para organizar dados desse tipo já foi apresentada. O que resta mostrar é o que acontecerá se uma coluna de dados armazenar duas variáveis em vez de uma. Nesse caso, teremos de executar uma operação de `pivot` ou `cast` da variável em colunas separadas.

```{python}
weather = pd.read_csv("Data/Cap_06/weather.csv")

# Dataframe no formato largo (wide)
print(weather.iloc[:5,:11])
```

Os dados meteorológicos incluem temperaturas mínima e máxima (valores `tmin` e `tmax` na coluna `element`, respectivamente) registradas para cada dia (`d1`, `d2`, `d3`,..., `d31`) do mês (`month`).

[A coluna element contém variáveis que devem ser submetidas a `cast`/`pivot` para que se transformem em novas colunas, e as variáveis de dia devem ser sujeitas à operação de `melt`, gerando valores de linha]{.underline}.

Novamente, não há nada de errado com os dados no formato atual. Eles simplesmente não estão em um formato apropriado para análises, embora esse tipo de formatação possa ser útil quando apresentamos dados em relatórios.
...
A seguir, devemos pivotear as variáveis armazenadas na coluna `element`. Esse processo é conhecido como *casting* ou *spreading* em outras linguagens estatística.

```{mermaid}
%%| label: fig-pivot
%%| fig-cap: "Transformando long (longo) em wide (largo) data."

graph TD
    A[Long Data] --pivot_table--> B[Wide Data]
```

Uma das principais diferenças entre `pivot_table` e `melt` é que [`melt` é uma função no Pandas]{.underline}, enquanto [pivot_table é um método que chamamos em um objeto `DataFrame`]{.underline}.
...
Sobre os parâmetros do `pivot_tabel`:

  * `index`: Define quais colunas não devem mudar. Elas identificam a linha (neste caso: `id`, `year`, `month`, `day`).

  * `columns`: Indica qual coluna contém os nomes que você quer transformar em novas colunas. No texto, é a coluna element (que contém os textos `tmax` e `tmin`).

  * `values`: Indica de onde virão os números (os dados reais) para preencher essas novas colunas. No exemplo, os valores vêm da coluna `temp`.

\newpage

Observando a tabela após a operação de pivot, percebemos que cada valor na coluna element está agora em uma coluna separada. É possível deixar essa tabela em seu estado atual, mas também podemos linearizar as colunas hierárquicas.
...
Quando você executa o `pivot_table`, o Pandas transforma as colunas que você passou no parâmetro `index` (como `id`, `year`, `month`, `day`) em um `MultiIndex` (índices hierárquicos). Visualmente, a tabela fica com as linhas "agrupadas" por essas categorias, o que pode dificultar manipulações posteriores.

O `.reset_index()` serve para:

   * Linearizar os dados: Ele retira essas colunas da posição de "índice" e as devolve para o corpo principal da tabela como colunas comuns.

   * Remover a hierarquia: Ele cria um novo índice numérico padrão (0, 1, 2...).

   * Facilitar o acesso: Torna a tabela "plata" (flat), que é o formato padrão do Tidy Data, facilitando filtros e exportações (como para Excel ou CSV).
...
### O Problema do "Destaque" (O Índice){.unnumbered}

Quando você usa o `pivot_table`, o Pandas decide que as colunas que você usou no parâmetro `index` (como `id`, `year`, `month`, `day`) são tão importantes que elas viram "etiquetas de linha".

Visualmente, a tabela fica assim:

   * As colunas id, ano, mês, dia ficam em negrito e em um nível diferente.

   * Elas não se comportam mais como colunas normais onde você pode fazer cálculos; elas viram apenas "nomes das linhas".

Isso é o que o texto chama de hierarquia. É ótimo para um relatório visual (fica bonito de ler), mas é péssimo para o computador processar depois.

### A Solução: `reset_index()` (A Reindexação){.unnumbered}

O `reset_index()` é como se você desse um comando de "voltem para o seu lugar!".

   * Transforma etiquetas em colunas: Ele pega aquelas informações que estavam "presas" no índice (em negrito/destaque) e as coloca de volta como colunas normais da tabela.

   * Cria um contador simples: No lugar delas, ele cria um índice novo, que é apenas uma contagem simples: 0, 1, 2, 3...

\newpage

## Várias unidades de observação em uma tabela (normalização)

Um dos modos mais simples de saber se várias unidades de observação estão representadas em uma tabela é observar cada um das linhas e prestar atenção se há alguma célula ou valor sendo repetido nas linhas.

Isso é muito comum em dados administrativos de educação do governo, em que informações demográficas dos estudantes são informações para cada aluno, para cada ano em que esse aluno estiver matriculado.

Vamos observar novamente os dados da Billboard que limpamos anteriormente.

```{python}
print(billboard_long.head())
```

Supanha que tenhamos criado um subconjunto de dados com base em uma faixa musical em particular.
...
* Análise do código:
   ...
   Ele realiza uma filtragem no DataFrame original (`billboard_long`).

   * `billboard_long.track == 'Loser'`: Procura na coluna de músicas (`track`) apenas as linhas onde o nome é 'Loser'.

   * `billboard_long[...]`: Seleciona essas linhas específicas.

   * `.head()`: Mostra apenas as 5 primeiras ocorrências desse filtro.

* Por que ele é importante para o `Tidy Data`?

   Ao isolar uma única música, fica visualmente claro o problema da redundância:

   * Você verá que o nome do artista, o nome da faixa e o tempo da música se repetem identicamente em todas as linhas.

   * A única coisa que realmente muda de linha para linha é a semana e a classificação (rating).

Podemos ver que essa tabela, na verdade, armazena dois tipos de dados: [a informação da faixa musical (`track`)]{.underline} e [a classificação semanal]{.underline}.

Seria melhor armazenar a informação da faixa em uma tabela separada. Dessa forma, as informações armazenadas nas colunas `year`, `artist`, `track` e `time` não estariam repetidas no conjunto de dados. Essa consideração é particularmente importante se os dados forem inseridos manualmente. Repetir os mesmos valores de modo contínuo durante a entrada de dados eleva o risco de haver dados inconsistentes.

\newpage

O que deveríamos fazer nesse caso é colocar `year`, `artist`, `track`, `time` e `date.entered` em um novo dataframe, com cada conjunto único de valores recebendo um `ID` único.

Podemos usar esse `ID` único em um segundo dataframe que represente a música, a data, o número da semana e a classificação.

Esse processo como um todo pode ser pensado como uma inversão dos passos de concatenação e combinação de dados.
...
* Análise do código:
   ...
   * O código cria um novo DataFrame chamado `billboard_song` selecionando apenas as colunas que descrevem a música (`year`, `artist`, `track`, `time`).

Sabemos que há entradas duplicadas nesse dataframe, portanto devemos descartar as linhas duplicadas.
...
* Análise do código:
   ...
   * `drop_duplicates()`: Varre o DataFrame `billboard_song` e remove todas as linhas que são exatamente iguais a uma linha anterior.

   * Resultado: Se a música "Loser" aparecia em 20 linhas (uma para cada semana), agora ela aparece em apenas uma linha.

\newpage

Podemos então atribuir um valor único para cada linha de dados.
...
* Análise do código:
   ...
   * `len(billboard_song)`: Conta quantas músicas únicas restaram após o `drop_duplicates`.

   * `range(...)`: Gera uma sequência numérica (0, 1, 2...) do mesmo tamanho da tabela.

   * `billboard_song['id']`: Cria uma nova coluna chamada id e atribui esses números a cada linha.

* Por que isso é crucial?

   * Chave Primária: Esse id agora é o "RG" de cada música.

   * Eficiência: Em vez de repetir "Artist: Beck, Track: Loser, Time: 3:55" em centenas de linhas na outra tabela, você usará apenas o número do `id`.

   * Organização Mental: Isso separa claramente quem é o objeto (a música) de qual é o evento (a classificação na Billboard).

\newpage

Agora que temos um dataframe separada com as músicas, podemos usar a coluna `id` recém-criada para fazer uma correspondência entre uma música e sua classificação semanal.
...
* O que acontece aqui:
   ...
   * O Pandas olha para a tabela original (`billboard_long`) e para a nova tabela de músicas (`billboard_song`) e diz: "Onde os valores de `year`, `artist`, `track` e `time` forem iguais nas duas tabelas, traga a coluna `id` para cá".
...
* Análise do código:
   ...
   * Filtragem de Colunas: Ele descarta as colunas textuais (`artist`, `track`, `time`) que agora estão seguras na outra tabela (`billboard_song`).

   * Referência por ID: Mantém apenas o id (o "RG" da música) para sabermos a qual faixa aquele rating se refere.

* Resultado Final:

   * Tabela 1 (`billboard_song`): Contém os dados fixos da música (Cadastro).

   * Tabela 2 (`billboard_ratings`): Contém o desempenho semanal (Transações).

\newpage

### Revisão normalização de tabelas banco de dados{.unnumbered}

As regras de normalização (Formas Normais) visam eliminar a redundância e proteger a integridade dos dados.

As três principais são:

1ª Forma Normal (**1FN**)

   * Regra: Eliminar grupos repetidos e garantir que cada campo contenha apenas valores atômicos (únicos).

   * Ação: Cada interseção de linha e coluna deve ter apenas um dado.

   * Exemplo: Se na coluna telefones você tem (11) 999, (11) 888, isso viola a **1FN**. Você deve separar em duas linhas ou duas colunas.

2ª Forma Normal (**2FN**)

   * Regra: Estar na **1FN** e garantir que todos os atributos dependam da chave primária total.

   * Ação: Eliminar dependências parciais (comum em chaves compostas).

   * Exemplo: Na tabela original, o `artist` dependia da música, mas não dependia da `week` (semana). Ao separar a música em uma tabela própria com seu próprio `ID`, garantimos que o artista esteja "amarrado" apenas ao que lhe cabe.

3ª Forma Normal (**3FN**)

   * Regra: Estar na **2FN** e garantir que nenhum atributo dependa de outro atributo que não seja a chave.

   * Ação: Eliminar dependências transitivas (um campo não-chave determinando outro campo não-chave).

   * Exemplo: Se você tem as colunas `cidade` e `estado`. O `estado` depende da `cidade`, e não diretamente do `ID` do usuário. Você criaria uma tabela de "Cidades" para não ter que repetir o estado toda vez que escrever o nome da cidade.

```{python}
#| echo: false
#| error: false
#| warning: false
#| label: tbl-normalvstidy
#| tbl-cap: Comparando Normalização SQL e Tidy Data

from IPython.display import Markdown
from tabulate import tabulate
table = [["**Normalização**","Integridade e economia de espaço (evita redundância).","Bancos de Dados Relacionais (SQL)."],
         ["**Tidy Data**","Facilidade de análise e visualização.","Ciência de Dados (R, Python, Excel)."]]
Markdown(tabulate(
  table, 
  headers=["Conceito","Foco Principal","Conceito Comum"],
  colalign=("left","left","left"),
  tablefmt="grid"
  ))
```
...
A última porção da organização dos dados relaciona-se à situação em que [o mesmo tipo de dado está espalhado por várias tabelas]{.underline}. Esse problema já foi abordado anteriormente, em que discutimos a concatenação e o `merge` de dados.

Um dos [motivos pelos quais os dados podem estar separados em vários arquivos]{.underline} é o [tamanho desses arquivos]{.underline}. Ao separar dados em várias partes, cada uma delas se torna menor. Isso pode ser bom se houver necessidade de [compartilhar dados na internet ou por email]{.underline}, pois muito serviços limitam o tamanho de um arquivo que poder ser aberto ou compartilhado. O [processo de coleta de dados]{.underline} é outro motivo pelo qual um conjunto de dados pode estar separado em várias partes. Por exemplo, um conjunto de dados separado contendo informações sobre ações no mercado poderia ser criado para cada dia. 

Como o `merge` e a concatenação já foram discutidos, [esta seção terá como foco as técnicas para carregar rapidamente várias fontes de dados e reuni-las]{.underline}.

Exemplo:

O Unified New York City Taxi and Uber Data (Dados unificados de táxi e Uber da cidade de Nova York) é uma boa opção para demonstrar esses processos.

O conjunto completo contém dados sobre mais de 1,3 Bilhão de corridas de táxi e de Uber na cidade de Nova York e esta organizado em mais de 140 arquivos.

Como demonstração, trabalharemos com apenas cinco desses arquivos de dados.

Quando os mesmos dados estão separados em várias partes, em geral, estas tem um padrão de nomeclatura estruturado associado a elas.

Em primeiro lugar, vamos fazer download dos dados. Não se preocupe muito com os detalhes do bloco de código a seguir. O arquivo `raw_data_urls.txt` contém uma lista de URLs, em que cada URL é o link de download para uma parte dos dados de táxi.

Começamos abrindo e lendo o arquivo e iterando pelas linhas do arquivo (isto é, cada URL de dados). Faremos o download somente dos cinco primeiros conjuntos de dados, pois os arquivos são bem grandes. Usaremos um pouco de manipulação de strings para criar o path em que os dados serão salvos e utilizaremos a biblioteca `urllib` para fazer o download de nossos dados.
 ...
 * `urllib`
   ...
   A urllib é o canivete suíço nativo do Python para lidar com URLs. Ela permite que seu código interaja com a internet sem precisar de navegadores.

   * O que faz: Abre, lê e manipula dados de links da web.

   * Principais usos:

     * Fazer requisições HTTP (baixar o conteúdo de uma página ou API).

     * Codificar parâmetros de busca (transformar espaços em %20, por exemplo).

     *  Analisar a estrutura de uma URL (separar o domínio do caminho).

   * Ponto importante: Embora seja muito poderosa e nativa, muitos desenvolvedores preferem a biblioteca externa `requests` por ser mais "amigável". Mas a `urllib` é quem manda nos bastidores.

 * `glob`

   A `glob` é o seu "detetive de arquivos". Ela serve para encontrar caminhos de arquivos e pastas seguindo padrões específicos, de forma muito mais simples do que listar tudo manualmente.

   * O que faz: Retorna uma lista de arquivos que correspondem a um padrão (usando wildcards ou "coringas").

   * Principais usos:

      * Listar todos os arquivos `.jpg` de uma pasta específica.

      * Encontrar arquivos que começam com "relatorio_" e terminam em .pdf.

      * Navegar por subdiretórios rapidamente.

   * Exemplo de padrão: `glob.glob("*.csv")` traria todos os arquivos Excel/CSV da pasta atual.
...
# 1. Ajustamos o caminho base usando o expanduser para o Ubuntu entender o '~'
base_path = os.path.expanduser("~/Programacao/python_dados/Data/Cap_06/varias_tabelas")

# 2. Caminho completo do arquivo TXT
arquivo_urls = os.path.join(base_path, "raw_data_urls.txt")

# 3. Abrimos o arquivo usando o caminho completo
with open(arquivo_urls, 'r') as data_urls:
    for line, url in enumerate(data_urls):
        if line == 5:
            break
        
        # Extrai o nome do arquivo (ex: tabela1.csv)
        fn = url.split('/')[-1]
        
        # Define onde salvar: EXATAMENTE na mesma pasta do TXT
        fp = os.path.join(base_path, fn)
        
        print(f"Baixando linha {line}: {url}")
        print(f"Salvando em: {fp}")
        
        # Realiza o download
        urllib.request.urlretrieve(url, fp)
```

Infelizmente os links estão quebrados. Esse código não funciona mais, mesmo adaptando. Essa parte será puramente teórica.
...
Explicando o código:
  ...
  * `with open(arquivo_urls, 'r') as data_urls:`
   ...
   Abre o seu arquivo .txt. o `with` é um "segurança". Ele garante que, assim que o código terminar de ler as URLs (ou se der algum erro no meio), o arquivo seja fechado automaticamente. Sem o `with`, o arquivo poderia ficar "preso" na memória do Ubuntu.

   * `for line, url in enumerate(data_urls):`

   Normalmente, um for leria apenas a url. O `enumerate` adiciona um contador automático. Na primeira volta, `line` vale 0, na segunda 1, e assim por diante. É por isso que você consegue usar o `if line == 5: break` para parar o download após 5 arquivos.

   * `fn = url.split('/')[-1]`

      * O `.split('/')` corta a frase em todos os lugares que tem uma barra, transformando-a em uma lista: `['https:', '', 'site.com', 'dados', 'tabela1.csv']`.

      * O `[-1]` pega o último item dessa lista.

      * Resultado: `fn` (filename) passa a ser apenas `tabela1.csv`.

   * `fp = os.path.join(base_path, fn)`
   
      * `fp` significa File Path (caminho do arquivo).

      * Em vez de você escrever manualmente `/home/sergio/.../tabela1.csv`, o Python junta o caminho da pasta (`base_path`) com o nome do arquivo (`fn`) de forma limpa.

   * `urllib.request.urlretrieve(url, fp)`
      * Esta é a função que efetivamente "teletransporta" o arquivo da web para o seu HD.

      * Ela precisa de dois argumentos:
        * De onde vem? (`url`)
        * Para onde vai? (`fp`).

\newpage

Nesse exemplo, todos os arquivos com dados brutos das corridas de táxi têm o padrão `fhv_tripdata_YYYY_XX.csv`, em que `YYYY` representa o ano (por exemplo, 2015) e `XX` representa o número da parte. Podemos usaruma função simples de correspondência de padrões da biblioteca `glob` em Python para obter uma lista com todos os nomes de arquivos que correspondam a um determinado padrão.

```
import glob

#Obtém uma lista de arquivos csv da pasta de dados das corridas de táxi de nyc
nyc_taxi_data = glob.glob('../data/fhv_*')
print(nyc_taxi_data)
```

Agora que temos uma lista dos nomes dos arquivos que queremos carregar, podemos carregar cada arquivo emum dataframe. É possível optar por corregar cada arquivo individualmente, como temos feito até agora.
...
Podemos observar nossos dados e ver como é possível empilhá-los (concatená-los) elegantemente uns sobre os outros.
...
Os dados podem ser concatenados exatamente como fizemos em capítulos anteriores.

```
# Formato de cada dataframe
print(taxi1.shape)
print(taxi2.shape)
print(taxi3.shape)
print(taxi4.shape)
...
# Formato final dos dados de táxi concatenados
print(taxi.shape)
```

Entretanto, salvar manualmente cada dataframe se tornará tedioso quando os dados estiverem divididos em muitas partes. Como uma abordagem alternativa, podemos automatizar o processo usando laços e `list comprehensions` (abrangência de listas).

\newpage

### Carregando vários arquivos usando um laço

Um modo mais fácil de carregar vários arquivos é criar primeiro uma lista vazia, usar um laço para iterar pelos arquivos CSV, carregá-los em um dataframe do Pandas e, por fim, concatenar o dataframe à lista.

```{mermaid}
%%| label: fig-concatmultdata
%%| fig-cap: "Carregando vários arquivos usando um laço."

graph TD
    A[Criar lista vazia] --> B[Laço iterar CSV]
    B --> C[Coloca em dataframe]
    C --concatenar--> D[dataframe a lista]
```

[O tipo de dado final que queremos é uma lista de dataframes porque a função `concat` aceita uma lista de dataframes para concatenar]{.underline}.

\newpage

```
# Criar uma lista vazia na qual a concatenação será feita
list_taxi_df = []

# Percorrer cada arquivo CSV em um laço
for csv_filename in nyc_taxi_data:
   # Você pode optar por exibir o nome do arquivo para depuração
   # print(csv_filename)

   # Carrega o arquivo CSV em um dataframe
   df = pd.read_csv(csv_filename)

   # Concatena o dataframe à lista que armazenará os dataframes
...
Explicando o código:
   ...
   * `list_taxi_df = []`: 
      ...
      Antes de começar a ler os arquivos, você reserva um espaço na memória do Python para guardá-los. Essa lista vazia é como uma pasta onde você vai colocar cada "folha" (DataFrame) que encontrar.

   * `for csv_filename in nyc_taxi_data`:

      * `nyc_taxi_data` é uma variável que contém os nomes dos seus arquivos (ex: `['viagens_jan.csv', 'viagens_fev.csv']`).

      * A cada volta do laço, o Python pega um desses nomes e coloca dentro da variável temporária `csv_filename`.

  * `df = pd.read_csv`:
  
      Aqui o Pandas entra em ação. Ele abre o arquivo físico que está no seu computador e o transforma em um objeto DataFrame na memória. É como se ele estivesse lendo o papel e criando uma tabela digital.

  * `list_taxi_df.append(df)`:
   
      Essa é a parte mais importante. O comando `.append()` pega o DataFrame que você acabou de criar e o coloca dentro daquela lista vazia do início.
...
```
# Exibe o início do primeiro dataframe
print(list_taxi_df[0].head())
```

Agora que temos uma lista de dataframes, podemos concatená-los.
...
```
# O mesmo resultado foi obtido em comparação com a carga manual e a concatenação?
print(taxi.equals(taxi_loop_concat))
```

\newpage

### Carregando vários arquivos usando uma `list comprehension`

Python tem um idiom para percorrer dados em um laço adicionando-os em uma lista; esse idiom se chama `list comprehension` (abrangência de lista). O laço apresentado antes, exibido novamente a seguir, sem os comentários, pode ser escrito como uma `list comprehension`.

Em programação, "idiom" (em português, idiomatismo) refere-se à maneira mais natural, eficiente e comum de escrever um código em uma linguagem específica.

Cada linguagem tem uma sintaxe própria, mas o "idiom" vai além: é o jeito "certo" ou "elegante" de resolver um problema usando as ferramentas nativas daquela linguagem.

   * O código não-idiomático: 
   
      Funciona, mas ignora as facilidades da linguagem (ex: usar um laço for manual com append para criar uma lista, como no seu primeiro exemplo).

   * O código idiomático: 
      
      Usa recursos como a `list comprehension` para ser mais conciso e, muitas vezes, mais rápido.

```
# O código do laço sem comentários
list_taxi_df = []
for csv_filename in nyc_taxi_data:
   df = pd.read_csv(csv_filename)
   list_taxi_df.append(df)
...
```
# O mesmo código em uma list comprehension
list_taxi_df_comp = [pd.read_csv(data) for data in nyc_taxi_data]
```

O resultado de nossa list comprehension é uma lista, assim como no exemplo anterior com um laço.

```
print(type(list_taxi_df_comp))
```

Por fim, podemos concatenar o resultado exatamente como fizemos antes.

```
taxi_loop_concat_comp = pd.concat(list_taxi_df_comp)

# Os dataframes concatenados são os mesmos?
...
Este capítulo explorou como podemos reformatar os dados para que estejam mais apropriados à análise, à visualização e à coleta de dados.

Aplicamos os conceitos do artigo `Tidy Data` (Dados organizados) de Hadley Wickham a fim de mostrar as diversas funções e métodos para reformatar nossos dados. Essa é uma habilidade importante porque algumas funções exigem que os dados estejam organizados em um determinado formato, como tidy data ou não, para que funcionem.

Saber como reformatar nossos dados é uma habilidade importante tanto para o cientista de dados quanto para o analista.

```{python}
#| echo: false
#| error: false
#| warning: false
#| label: tbl-pivotandunpivot
#| tbl-cap: Métodos para transformação largo (wide) e longo (long)

from IPython.display import Markdown
from tabulate import tabulate
table = [["**Objetivo**","Transformar colunas em linhas (**Unpivot**).","Transformar linhas em colunas (**Pivot**)."],
         ["**Direção**","De Largo (Wide) para Longo (Long).","De Longo (Long) para Largo (Wide)."],
         ["**Tidy Data**","Formato ideal para análise e gráficos.","Formato ideal para relatórios e tabelas dinâmicas."],
         ["**Resultado**","Aumenta o número de linhas.","Reduz o número de linhas e cria novos cabeçalhos."],
         ["**Sintaxe**", "Função do pandas (`pd.melt(df)`)", "Método do DataFrame (`df.pivot_table()`)"]]
Markdown(tabulate(
  table, 
  headers=["Recurso","`pd.melt()`","`.pivot_table()`"],
  colalign=("left","left","left"),
  tablefmt="grid"
  ))
```
