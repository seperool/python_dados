---
title: "Estudo de Python e dados I"
subtitle: "Introdução: Básico de DataFrame Pandas, Estrutura de dados Pandas e Visualização com Pandas, Matplotlib e Seaborn"
...
Markdown(tabulate(
  table, 
  headers=["Tipo do Pandas","Tipo de Python","Descrição"],
...
Markdown(tabulate(
  table, 
  headers=["Método para obtenção de subconjuntos","Descrição"],
  colalign=("left","left")
))
```
...
* `axes1.boxplot(...)`:
   ...
   `labels = ['Female', 'Male']`:
   Atribui os rótulos correspondentes a cada boxplot na ordem em que os dados foram passados, garantindo que você saiba qual caixa representa qual grupo.
...
Para mostrar o processo de plotar dados multivariados, vamos nos basear no nosso gráfico de dispersão anterior. Se quiséssemos adcionar outra variável, por exemplo, `sex`, uma opção seria colorir os pontos com base no valor da terceira variável.

Se quiséssemos adicionar uma quarta variável, poderiamos acrescentar tamanho aos pontos. A única ressalva em usar o tamanho como uma variável é que os seres humanos não são muito bons para diferenciar áreas. É claro que, se houver um ponto enorme ao lado de um ponto minúsculo, sua informação será evidente, mas diferenças menores são difíceis de distinguir e poderão deixar a sua visualização confunsa. Uma maneira de reduzir a confusão é adicionar certa dose de transparência aos pontos individuais, de modo que muitos pontos sobrepostos serão exibidos como uma região mais escura de uma plotagem, em comparação com áreas menos cheias.

Ou seja, tamanho e transparencia são recursos interessantes, porem cor é mais recomendados para destacar as variáveis. A cor é o recurso mais direto para adicionar uma variável extra (terceira) em um gráfico de dispersão, enquanto o tamanho, embora útil para uma quarta variável, deve ser usado com cautela devido à dificuldade humana em diferenciar áreas. A transparência serve como um ajuste para aumentar a clareza, revelando regiões de maior concentração de pontos sobrepostos.

Como regra geral, cores diferentes são muito mais fáceis de distinguir do que variações de tamanho. Se você tiver de usar áreas para representar diferenças entre valores, certifique-se de que estará realmente plotando áreas relativas. Uma armadilha comum é mapear um valor para o raio de um círculo nas plotagens, mas, como a fórmula de um círculo é $\pi r^2$, suas áreas na verdade estarão baseadas em uma escala quadrática, Isso não só é enganoso como também é incorreto.

Ao utilizar o tamanho (área) dos pontos para representar valores de dados, uma 'armadilha' comum e incorreta é mapear o valor dos dados ($V$) diretamente para o raio ($r$) de um círculo. Como a área do círculo é determinada pela fórmula $A=\pi r^2$, se $r=V$, a área plotada crescerá em escala quadrática ($A=\pi V^2$) em relação ao valor original, e não linearmente. Isso é enganoso. A solução, de forma análoga, é mapear o valor ($V$) para a área desejada ($A_{desejada} ​= V$) e, então, calcular o raio necessário ($r=\sqrt{\frac{V}{π}​}$).

Cores também são difíces de escolher. Os seres humanos não percebem tons em uma escala linear, portanto será necessário pensar cuidadosamente ao selecionar as paletas de cores. Felizmente o `matplotlib` e o `seabor` têm o próprio conjunto de paletas de cores, e ferramentas como o `colorbrewer` podem ajudar você a escolher boas paletas.

\newpage

A figura \textcolor{red}{x} usa cores para adicionar uma terceira variável, `sex`, em nosso gráfico de dispersão.
...
Podemos pensar a bibliteca `matplotlib` como a principal ferramenta básica de plotagem em Python. O `seaborn` é baseada na `matplotlib` e oferece uma interface de nível mais alto para **gráficos estatísticos**. Ele provê uma interface para criar visualizações mais bonitas e complexas com menos linhas de código.
...
Essencialmente, são criadas usando uma distribuição normal centralizada emcada ponto de dado, e então suavizado as plotagens que se sobrepõem de modo que a área sob a curva seja 1.
...
#### Plotagem de contadores (plotagem de barras)
...
Esse é um exemplo do `seaborn` funcionando com base na `matplotlib`, pois `hexbin` éuma função da `matplotlib`.
...
Sobre o código:
  ...
  - Na versão mais antiga do `kdeplot` (no livro) ao invés de usar `x`, `y` e `fill`, era respectivamente `data`, `data2` e `shade`, porém o significando é o mesmo.
...
As plotegens de violino (`violin plots`) podem mostrar os mesmos valores que uma plotegem de caixa, porém plotam as "caixas" com uma estimativa de **densidade por kernel**.
...
Uma desvantagem ao usar `pairplot` esta no fato de haver informações reduntantes, isto é, a parte superior da visualização é igual a metade inferior.
...
Considere um exemplo de como o `seaborn` trabalha com chamadas de função da `matplotlib`. Se consultar a documentação de `lmplot`, você verá que ela aceita parâmetros de nomes `catter`, `line scatter`, `line_kws`.
...
anscombe_plot = sns.lmplot(
    x='x',                   # Eixo horizontal
    y='y',                   # Eixo vertical
    data=anscombe,           # Fonte dos dados
    fit_reg=False,           # Remove linha de regressão
    col='dataset',           # Cria subplots por categoria
    col_wrap=2,              # Limita a 2 colunas por linha
    height=2,                # Altura de cada quadro (em polegadas)
    aspect=1                 # Razão largura/altura (1 = quadrado)
)
```
...
facet.map(                   # Aplica a função de plotagem em cada subgráfico
  sns.distplot,              # Define o tipo de gráfico (distribuição)
  'total_bill',              # Define a variável do eixo X
...
facet = facet.map(          # Mapeia o gráfico de dispersão para a grade
  plt.scatter,              # Define a função de plotagem (matplotib)
...
#| warning: false
#| label: fig-seabornmv
#| fig-cap: "Plotagem do seaborn com facetas criadas manualmente contendo diversas variáveis."
fig = sns.lmplot(
...
#| warning: false
#| label: fig-seaborn2v
#| fig-cap: "Plotagem do seaborn com facetas criadas manualmente com duas variáveis."
facet = sns.FacetGrid(
...
```{python}
#| warning: false
#| label: fig-seaborncatviolin
#| fig-cap: "Plotagem do seaborn com facetas criadas manualmente com duas variáveis violin."
facet = sns.catplot(
...
```{python}
#| warning: false
#| label: fig-seaborncatbox
#| fig-cap: "Plotagem do seaborn com facetas criadas manualmente com duas variáveis boxplot."
facet = sns.catplot(
...
```{python}
#| warning: false
#| label: fig-histpandasS
#| fig-cap: "Histograma de uma Series do Pandas."
fig, ax = plt.subplots()
...
```{python}
#| warning: false
#| label: fig-histpandasdf
#| fig-cap: "Histograma de um DataFrame do Pandas."
...
```{python}
#| warning: false
#| label: fig-kdepandasdf
#| fig-cap: "Plotagem de KDE do Pandas."
fig, ax = plt.subplots()
...
```{python}
#| warning: false
#| label: fig-scatterpandasdf
#| fig-cap: "Gráfico de dispersão do Pandas."
fig, ax = plt.subplots()
...
```{python}
#| warning: false
#| label: fig-hexbinpandasdf
#| fig-cap: "Plotagem hexbin Pandas."
fig, ax = plt.subplots()
ax = tips.plot.hexbin(
  x='total_bill',
  y='tip',
...
```{python}
#| warning: false
#| label: fig-sizehexbinpandasdf
#| fig-cap: "Plotagem hexbin do Pandas com tamanho de grade modificado."
fig,ax = plt.subplots()
ax = tips.plot.hexbin(
  x='total_bill',
  y='tip',
...
```{python}
#| warning: false
#| label: fig-boxpandasdf
#| fig-cap: "Plotagem de caixas do Pandas."
fig, ax = plt.subplots()
ax = tips.plot.box(
  ax=ax
)
...
Os estilos incluídos no `seaborn` são:
* `darkgrid`
* `whitegrid`
* `dark`
* `white`
...
A @fig-stylebasicsns mostra uma plotagem básica, enquanto a @fig-stylewhitegridsns exibe uma plotagem com estilo `whitegrid`.

```{python}
#| warning: false
#| label: fig-stylebasicsns
#| fig-cap: "Estilo básico do seaborn."
...
```{python}
#| warning: false
#| label: fig-stylewhitegridsns
#| fig-cap: "Estilo whitegrid do seaborn."
...
#| warning: false
#| label: fig-stylessns
#| fig-cap: "Todos os estilos seaborn."
fig=plt.figure()
