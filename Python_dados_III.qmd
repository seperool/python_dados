...
A @tbl-sintaxe-regex e @tbl-especiais-regex mostram parte da sintaxe das RegEx e os caracteres especiais que serão usados nesta seção.
...
#| error: false
#| warning: false
#| label: tbl-especiais-regex
#| tbl-cap: Caracteres especiais de RegEx
...
table_referencia = [
    [r"`\d`", "Um digito."],
    [r"`\D`", "Qualquer caractere que NÃO seja um digito (oposto de `\d`)."],
    [r"`\s`", "Qualquer caractere de espaço em branco."],
    [r"`\S`", "Qualquer caractere que NÃO seja um espaço em branco (oposto a `\s`)."],
    [r"`\w`", "Caracteres de palavras."],
    [r"`\W`", "Qualquer caractere que NÃO seja um caracteres de palavras (oposto a `\w`)."]
]

# Gerando a tabela formatada
Markdown(tabulate(
  table_referencia, 
  headers=["Sequência", "Descrição"],
  colalign=("left", "left"),
  tablefmt="grid"
))
```

Para usar expressões regulares, escreva uma string contendo o padrão **RegEx** e especifique uma string para a correspondência com o padrão. Várias funções do módulo `re` podem ser usadas para cuidar de necessidades específicas. Algumas tarefas comuns estão apresentadas na \textcolor{red}{tabela x}.
